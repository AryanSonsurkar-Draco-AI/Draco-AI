...
def process_command(raw_cmd: str) -> str:
    ...
    if "motivate me" in cmd:
        reply = "Bro, you’re built for greatness. Lock in and show them why you are different."
        speak(reply)
        return reply
    
    if "Am I improving?" in cmd:
        reply = "Absolutely. Your progress is visible. Just keep stacking these small wins."
        speak(reply)
        return reply
    ...
    if "Why should i use you?" in cmd:
        reply = "Because I’m built to back you up like the most reliable anime partner."
        speak(reply)
        return reply
    
    if "i am tired" in cmd or "I need rest" in cmd:
        reply = "Rest for a bit, then come back stronger. You never quit, remember that."
        speak(reply)
        return reply

    if "who are you" in cmd or "what's your name" in cmd:
